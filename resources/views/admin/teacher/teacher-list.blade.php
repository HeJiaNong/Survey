...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="{{ route('user_search_store') }}" method="post">
            ...
            <input @if(isset($teachername)) value="{{ $teachername }}"  @endif type="text" name="username" placeholder="请输入用户名" autocomplete="off" class="layui-input">
    ...
    <xblock>
        ...
        <span class="x-right" style="line-height:40px">共有数据：{{ $teachers->total() }} 条</span>
    </xblock>
    <table class="layui-table">
        ...
        <tbody>
            @foreach($teachers as $teacher)
                <tr>
                    <td>
                        <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id={{ $teacher->id }}><i
                                    class="layui-icon">&#xe605;</i></div>
                    </td>
                    <td>{{ $teacher->id }}</td>
                    <td>{{ $teacher->name }}</td>
                    ...
                    <td class="td-manage">
                        @switch($teacher->status)
                            @case(1)
                        <a onclick="member_stop(this,{{ $teacher->id }})" href="javascript:;" title="停用">
                            <i class="layui-icon">&#xe601;</i>
                        </a>
                            @break
                            @case(0)
                            <a onclick="member_stop(this,{{ $teacher->id }})" href="javascript:;" title="启用">
                                <i class="layui-icon">&#xe62f;</i>
                            </a>
                            @break
                        @endswitch
                        <a title="编辑" onclick="x_admin_show('编辑','{{ route('users_edit_page',$teacher->id) }}',600,400)" href="javascript:;">
                            <i class="layui-icon">&#xe642;</i>
                        </a>
                        <a onclick="x_admin_show('修改密码','{{ route('edit_passwd_page',$teacher->id) }}',600,400)" title="修改密码" href="javascript:;">
                            <i class="layui-icon">&#xe631;</i>
                        </a>
