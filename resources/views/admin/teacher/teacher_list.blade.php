...
@section('body')
    ...
    <div class="x-body">
        <div class="layui-row">
            <form class="layui-form layui-col-md12 x-so" action="{{ route('admin_teacher_search_post') }}" method="post">
                {{ csrf_field() }}
        ...
        <xblock>
            ...
            <button class="layui-btn" onclick="x_admin_show('添加用户','{{ route('admin_teacher_addPage_get') }}',600,400)"><i
            ...
            @foreach($dataset as $data)
                <tr>
                    ...
                    <td class="td-manage">
                        @switch($data->status)
                            @case(1)
                            <a onclick="member_stop(this,'{{ route('admin_teacher_status_get',$data->id) }}')" href="javascript:;" title="停用">
                                <i class="layui-icon">&#xe601;</i>
                            </a>
                            @break
                            @case(0)
                            <a onclick="member_stop(this,'{{ route('admin_teacher_status_get',$data->id) }}')" href="javascript:;" title="启用">
                                <i class="layui-icon">&#xe62f;</i>
                            </a>
                            @break
                        @endswitch
                        <a title="编辑" onclick="x_admin_show('编辑','{{ route('admin_teacher_editPage_get',$data->id) }}',600,400)" href="javascript:;">
                            <i class="layui-icon">&#xe642;</i>
                        </a>
    ...
    <script>
        ...
        function delAll(argument) {
            ...
            if (data.length == 0){
                ...
                // layer.confirm('确认要停用' + "" + '吗？', function (index) {
                layer.confirm('确认要停用' + ""+data + '吗？', function (index) {
                    //捉到所有被选中的发异步进行删除，
                    $.ajax({
                        type: "get",
                        url: "http://www.survey.test/admin/teacher/status_bulk/"+data,
                        dataType: "json",
                        //服务器返回执行操作的状态
                        success: function(status) {
