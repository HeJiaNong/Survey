...
<main>
    ...
    <form action="{{ route('home_wordShow') }}" method="post" data-student-src="/index/student/index.html" data-teacher-src="/index/teacher/index.html" data-staff-src="/index/staff/index.html" data-comp-src="/index/comp/index.html">
        {{ csrf_field() }}
        <div class="select-list">
            <label class="select-election" data-level="1">
                查询方式：
                <!--这个name被我使用js控制,请不要修改-->
                <select id="select-type" name="word" class="per">
            ...
            <label class="select-election select-class hidden" data-level="2">
                选择班级：
                <!--这个name被我使用js控制,请不要修改-->
                <select name="grade" class="class_selected grade_name">
...
<script type="text/javascript">
    ...
    form.addEventListener('submit', function (e) {
        ...
        */
        ...
        {{--location = '{{ route('home') }}'+'/word_show'+'/'+data['type'];--}}
        /*
             根据请求的方式,跳转到不同的页面
         */
        // if(data['type']==="1"){
...
<script>
    ...
    $(".per").change(function(){
        var id = $(".per").val();
        var url = '{{ route('home') }}'+'/getGrade/'+id;
        console.log(url);
        $.ajax({
            async: true,    //异步
            type: "GET",
            ...
            success: function(data) {
                ...
                $.each(data, function(key, val) {
                    $('.grade_name').append('<option value="'+key+'">'+val+'</option>');
                });
            },
            error:function (data) {
                console.log('操作失败!');
